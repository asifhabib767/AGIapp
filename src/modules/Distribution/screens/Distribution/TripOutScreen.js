...
const TripOutScreen = (props) => {
  ...
  const tripCancel = (strCode, intTripId) => {
    ...
    if (rollback) {
      ...
      props.navigation.navigate('tripOut');
    }
  };

  const checkOut = async (item) => {
    setisLoading(true);
    const tripCompelte = await getOutComplete(item);

    if (tripCompelte.status) {
      setisLoading(tripCompelte.isLoading);
      Alert.alert('Gate Out Completed', encryptCode(tripCompelte.message));
    } else {
      Alert.alert('Warning', tripCompelte.message);
      setisLoading(false);
    }
    setisLoading(false);
    props.navigation.navigate('tripOut');
