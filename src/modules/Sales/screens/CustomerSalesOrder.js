...
import {getShippingPoints} from '../service/sales/ShippingPoint';
...
import {
  ...
  getCustomerTypes,
  getCustomerSalesTypeDetails,
} from '../service/distributor/Distributor';
import AsyncStorage from '@react-native-community/async-storage';
import {
  salesOrderCreate,
  getCustomerList,
  getCustomerLoadingProfile,
...
import {getCustomerDetails} from '../service/customers/CustomerService';
import {FlatList} from 'react-native-gesture-handler';

class CustomerSalesOrder extends Component {
  constructor(props) {
    super(props);
    this.state = {
      intUserID: '',
  ...
  componentDidMount = async () => {
    ...
    const shipping_points = await getShippingPoints();

    const customerDetailsData = await getCustomerDetails();
    ...
    let all_delivery_points = await getDeliveryPoints(this.state.email, true);

    let customer_types = await getCustomerTypes(4); // Unit 4
    let getCustomerProfile = await getCustomerLoadingProfile();
    let getCustomerSalesType = await getCustomerSalesTypeDetails();

    let intSalesTypeId = parseInt(getCustomerSalesType.intTypeID);
    console.log('getCustomerSalesType', intSalesTypeId);
    ...
    this.setState({
      ...
      intSalesOffId: getCustomerProfile.intSalesOffId,
      strSalesOfficeName: getCustomerProfile.salesofficename,
      intCusType: getCustomerProfile.intCusType,
      intCustomerType: getCustomerProfile.intCusType,
      strTypeName: getCustomerProfile.strTypeName,
      intSalesTypeId,
    });
  ...
  searchDeliveryPoints = (searchText) => {
    // if (this.state.intShipPointId == '') {
    //   Alert.alert('Error', 'Plese select shipping point');
    //   return false;
    // }
    ...
    const searched_delivery_points = this.state.all_delivery_points.filter(
      function (item) {
        const strCusName = item.strDisPointName + ' ' + item.intDisPointId;
        // const itemData = item.strCusName ? item.strCusName.toUpperCase() : ''.toUpperCase();
  ...
  submitOrder = async () => {
    ...
    const singleProductData = {
      ...
      logisId: parseInt(this.state.intVehicleVarId), // vehicle bar id
      prom: 0,
    ...
    const ysnSubmitByCustomer = true;
