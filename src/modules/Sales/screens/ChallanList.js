...
import {getChallanList} from '../service/sales/ChallanService';
import AsyncStorage from '@react-native-community/async-storage';

import * as DateClass from '../../Master/Util/DateConfigure';

export default class ChallanList extends Component {
  state = {
    sortBy: '1',
    challanList: [],
  ...
  initializeData = async () => {
    let challanList = await getChallanList(
                  ...
                  <View style={[styles.distributorDetails]}>
                    <View style={{flexBasis: '65%'}}>
                      <Text style={[styles.distributerName]}>
                        {' '}
                        Challan #{item.strchallan}{' '}
                      </Text>

                      <Text style={{marginLeft: 8, fontWeight: 'bold'}}>
                        Product Name: {item.strProductName}{' '}
                      </Text>
                      <Text style={{marginLeft: 8}}>
                        Customer: {item.strCustName}
                      </Text>
                      <Text style={{marginLeft: 8}}>Price: {item.rate}</Text>
                      <Text style={{marginLeft: 8}}>
                        Challan Vehicle: {item.strvheicle}
                      </Text>
                      <Text style={{marginLeft: 8}}>
                        Challan Date:{' '}
