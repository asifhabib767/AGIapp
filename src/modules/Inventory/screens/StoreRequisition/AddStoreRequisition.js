...
const AddStoreRequisition = (props) => {
  ...
  const submit = async () => {
    ...
    let response = await AddStoreRequisitionAction(state);
    console.log('response store', response);
    if (response.status) {
      // Alert.alert('Success', `${response.data.message}`);
      Alert.alert('Success', 'Store requisition crearted');
      props.navigation.navigate('storeRequisition');
    } else {
      Alert.alert('Warning', 'Store Requisition did not created successfully!');
    }
  };
  ...
  const searchText = async (searchRequestText) => {
    ...
    if (searchRequestText.length > 0) {
      let getSearchValue = {};
      getSearchValue.data = state.itemListData;
  ...
  return (
    <KeyboardAvoidingView style={[styles.container]}>
      <ScrollView>
        <View>
          <Header title="Store Requisition" />
