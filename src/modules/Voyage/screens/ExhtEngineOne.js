...
import { exhtEngineTwoSubmit } from "../actions/ExhtEngineAction";

const ExhtEngineOne = (props) => {
  ...
  const submit = async () => {
    ...
    let response = await exhtEngineTwoSubmit(props.route.params, state);
    if (response.status) {
  ...
  return (
    <KeyboardAvoidingView style={[styles.container]}>
      <ScrollView>
        <View>
          <Header title="Exht Engine 2" />

          <View style={[styles.container]}>
            <VoyageHeader headerProps={state.voyagePropsData} />
            <View>
              <View>
                <Text style={[styles.labelstyletwo]}>EXHT.ENGINE 2</Text>
              </View>
