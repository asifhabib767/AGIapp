...
import { exhtEngineOneSubmit } from "../actions/ExhtEngineAction";
import { Input } from "@ui-kitten/components";

const ExhtEngine = (props) => {
  const [state, setState] = React.useState({
    system_lang: "bn",
    refreshing: false,

    dceMainEngineFuelRPM: 0,
    dceRH: 0,
    dceLoad: 0,
    dceExhtTemp1: 0,
    dceExhtTemp2: 0,
  ...
  const submit = async () => {
    if (state.dceMainEngineFuelRPM === 0) {
      Alert.alert("Warning", "Please give Main Engine Fuel RPM");
      return false;
    }
    if (state.dceRH === 0) {
    ...
    let response = await exhtEngineOneSubmit(props.route.params, state);
    if (response.status) {
  ...
  return (
    <KeyboardAvoidingView style={[styles.container]}>
      <ScrollView>
        <View>
          <Header title="Exht Engine 1" />

          <View style={[styles.container]}>
            <VoyageHeader headerProps={state.voyagePropsData} />
            <View>
              <Text style={[styles.enginetextstyleone]}>
                Main Engine Fuel Control/Pitch
              </Text>
            </View>
            <View>
              <View style={{ flex: 1, flexDirection: "row", marginTop: 15 }}>
                <View
                  style={{ flexBasis: "25%", marginRight: 10, marginLeft: 10 }}
                >
                  <View>
                    <View>
                      <Text
                  ...
                  <View style={[styles.inputBoxStyle]}>
                    <View>
                      <TextInput
                        ...
                        value={state.dceMainEngineFuelRPM}
                        onChangeText={(value) =>
                          handleInputChange("dceMainEngineFuelRPM", value)
                        }
                      />
                    </View>
                  </View>
              ...
              <View>
                <Text style={[styles.labelstyletwo]}>EXHT.ENGINE 1</Text>
              </View>
