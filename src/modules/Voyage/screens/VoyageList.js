...
import qr from "../images/attendance.png";
import {
...
import { getVoyageListAction } from "../actions/VoyageAction";
...
const VoyageList = (props) => {
  ...
  const getInitialData = async () => {
    ...
    // setState({ searchOrderList: voyageList });

    let response = await getVoyageListAction();
    setState({ searchOrderList: response.data });
  };

  const searchText = (searchRequestText) => {
    let cloneObj = { ...state };
    if (searchRequestText.length > 0) {
      const newData = voyageList.filter(function (item) {
        const itemData =
      ...
      cloneObj.searchOrderList = voyageList;
        ...
        <View style={{ backgroundColor: "#F3F6FA" }}>
          <View style={[styles.postionbox]}>
            <HeaderButton
              title="Voyage List"
              btnName="Add"
              url={() => props.navigation.navigate("createVoyage")}
            />
          </View>
                    ...
                    >
                      ...
                      <View style={styles.itemList}>
                        ...
                        <View style={{ flexBasis: "38%" }}>
                          <Text style={{fontWeight: "bold"}}>Voyage No {" "}</Text>
