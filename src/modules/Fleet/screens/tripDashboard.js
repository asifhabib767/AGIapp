...
export default class tripDashboard extends Component {
  ...
  initializeList = async () => {
    ...
    let is_driver = (await AsyncStorage.getItem('is_driver')) || 'none';
    // Alert.alert('Driver', is_driver);
    ...
    return (
      <ScrollView
        ...
        <SafeAreaView>
          ...
          {this.state.pendingTripList.map((item, index) => (
            <TouchableOpacity
              // onPress={() => Actions.MyTrip({itemData: item})}
              onPress={() =>
                this.props.navigation.navigate('mytrip', {itemData: item})
              }
              key={index}>
              <Animatable.View animation="fadeInUp" key={index}>
                    ...
                    <View
                      ...
                      <View style={{flexBasis: '65%'}}>
                        <Text style={[styles.tripNos]}>
                          ট্রিপ কোডঃ {item.TripCode}{' '}
                        </Text>
                        <Text style={[styles.tripNos]}>
                          ট্রিপ আইডিঃ #{item.tripID}{' '}
                        </Text>
                        {index === 0 && (
