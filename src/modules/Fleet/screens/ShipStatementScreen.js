...
import {getShipData} from '../service/shippingStatement/shippingStatementService';
import {generateStringDateFromDate} from '../Util/DateConfigure';

export default class ShipStatementScreen extends Component {
  state = {
    all_vehicles: [],
  ...
  onShowShipDetails = async () => {
    ...
    let singleShipData = await getShipData(
      startDate,
      endDate,
      vehicleId,
      intDriverEnroll,
        ...
        <SafeAreaView style={styles.container}>
          <View style={{flex: 1, flexDirection: 'row'}}>
            <View style={{flexBasis: '70%'}}>
              <Text style={[styles.headingOne]}> শিপ স্টেটমেন্ট </Text>
            </View>
          </View>
          ...
          <View>
            <LinearGradient
              ...
              <TouchableOpacity onPress={this.onShowShipDetails}>
                <Text style={styles.buttonText}>স্টেটমেন্ট দেখুন &nbsp; </Text>
              </TouchableOpacity>
            </LinearGradient>
          </View>
          <View style={[styles.selectBox]}>
            {this.state.singleShipData.map((item, index) => (
              <View
                style={{
                  flex: 1,
                  flexDirection: 'row',
                ...
                <View style={{flexBasis: '58%'}}>
                  <Text style={[styles.shipmentNo]}>
                    সময়:{generateStringDateFromDate(item.dteinsertime)}
                  </Text>
                  <Text style={[styles.shipDate]}>
                    লাইটার এর নাম:{item.strParentvheicle}{' '}
                  </Text>
                  <Text style={[styles.shipDate]}>
                    ঠিকানা:{item.strToaddress}{' '}
                  </Text>
                  <Text style={[styles.shipDate]}>
                    চালান নাম্বার:{item.strchallan}{' '}
                  </Text>
                  <Text style={[styles.shipDate]}>
                    প্রতি ব্যগ রেট:{item.PerbagRate}{' '}
                  </Text>
                </View>
                <View style={{flexBasis: '20%'}}>
                  <Text style={[styles.shipmentNo]}> পরিমান</Text>
                  <Text style={[styles.amount]}> {item.numpieces} ব্যাগ</Text>
                </View>

                <View style={{flexBasis: '22%'}}>
                  <Text style={[styles.shipmentNo]}> টোটাল</Text>
                  <Text style={[styles.amount]}>
                    {item.monTotalAmount} টাকা
                  </Text>
                </View>
              </View>
            ))}
