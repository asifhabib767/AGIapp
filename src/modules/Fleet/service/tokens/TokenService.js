...
export async function generateToken(username, password) {
  ...
  console.log('enter login generate');

  tokenData = await Axios.get(
    `http://api2.akij.net:8066/api/DataForAppsUserIDNPasswdVerification/GeDataForAppsUserIDNPasswdVerification?intpart=1&username=${username}&passwd=${password}&intunitid=4`,
    loginData,
    {headers: headers},
  )
