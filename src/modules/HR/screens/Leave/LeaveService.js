...
export async function getLeaveList() {
  ...
  if (api_get_leave_list.length !== 0) {
    await Axios.get(`${api_get_leave_list}?EmployeeId=${intEmployeeId}`, {
      token: getToken(),
    }).then((res) => {
...
export async function getLeaveType() {
  ...
  if (api_get_leave_type.length !== 0) {
    await Axios.get(`${api_get_leave_type}?EmployeeId=${intEmployeeId}`, {
      token: getToken(),
    }).then((res) => {
...
export async function PostLeave(addtype, fromDate, endDate, reason, address) {
  ...
  let strContactNo1 = await getUserData();
  ...
  let leavePost = await Axios.post(
    `${api_post_leave_create}?EmpCode=${strEmployeeCode}&LeaveTypeID=${addtype}&FromdDate=${fromDate}&ToDate=${endDate}&LeaveReason=${reason}&Address=${address}&PhoneNumber=${strContactNo1}&ActionBy=${intEmployeeId}`,
    {},
  )
...
export async function DeleteLeave(ApplicationId) {
  ...
  Axios.post(
    `${api_leave_delate}?EmpCode=${strEmployeeCode}&ApplicationId=${ApplicationId}&ActionBy=${intEmployeeId}`,
    {},
  )
    .then(function (response) {
