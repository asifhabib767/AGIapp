import { api_get_attendance_list, api_post_attendance } from '../../../config.json';
...
export async function getAttendanceData() {
  ...
  if (api_get_attendance_list.length !== 0) {
    await Axios.get(
        `${api_get_attendance_list}?EmployeeId=${intEmployeeId}`,
      {
        token: getToken(),
      },
    ).then(res => {
...
let attendance=Axios.post(`${api_post_attendance}?EmployeeId=${intEmployeeId}&Lang=${longitude}&lat=${latitude}&JobstationId=${intJobStationId}`, {})
