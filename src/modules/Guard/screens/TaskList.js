...
import { addTask, getIncidentType } from "../service/task/TaskService";
...
export default class TaskListScreen extends Component {
  state = {
    ...
    checkpointName: this.props.route.params.data,
  };
  ...
  submit = async () => {
    ...
    if (checkedData.length != 0) {
      let response = await addTask(checkedData);
    } else {
            ...
            <View style={[styles.areaBox]}>
              <View>
                <Text style={[styles.headerLabel]}>
                  {translate("task_list", this.state.system_lang)}
                </Text>
              </View>
