...
export async function getRoute() {
  ...
  if (
    intEmployeeId == 439590 ||
    intEmployeeId == 392407 ||
    intEmployeeId == 412378 ||
    intEmployeeId == 318035 ||
    intEmployeeId == 422905
  ) {
  ...
  try {
    if (api_get_route.length !== 0) {
      await Axios.get(`${api_get_route}?intID=${intEmployeeId}`, {
        headers: headersData,
      }).then((res) => {
        data = res.data;
...
export async function getBustinessType() {
  ...
  try {
    if (api_business_type.length !== 0) {
      await Axios.get(`${api_business_type}`, { headers: headersData }).then(
        (res) => {
          data = res.data;
...
export async function updateOutletRegistration(
  intRouteID,
  strRouteName,
  strOutletName,
  strOwnerName,
