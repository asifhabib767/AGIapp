...
import { View, Text, Image, StyleSheet, SafeAreaView, ScrollView, TextInput, TouchableOpacity, Linking } from 'react-native';
...
class Outlet extends Component {
    ...
    async componentDidMount ()  {
        console.log('ImagePath:', this.props.outlet.strOutletImagePath);
    ...
    render() {
        const { intRouteID, intOutletID, strRouteName, strOutletName, strOwnerName, strMobileNumber, strBusinessType, dteDateOfBirth, dteMarriageDate, strEmailAddress, strLatitude, strLongitude, strOutletImagePath , strOutletAddress, strContactType} = this.props.outlet;
        console.log('Render', this.props.outlet.strOutletImagePath);
            ...
            <SafeAreaView >
                <View style={[styles.AccountDetailsArea]}>
                    ...
                    <View style={{flex:1,flexDirection:'row'}}>
                        ...
                        <View style={{flexBasis:'70%',alignItems:'flex-end',justifyContent:'center',}}>
                    ...
                    <View style={{ flex: 1, flexDirection: "row", }}>
                        ...
                        <View style={{ flexBasis: '10%', marginRight: 10, marginTop: 13 }}>
                            <Image source={shop} />
                        </View>
                        <View style={{ flexBasis: '60%', paddingTop: 20, paddingLeft: 10 }}>
                            <Text style={[styles.statusAddress]}> {strOutletName}</Text>
                            <Text style={[styles.statusLeave, { fontSize: 18, fontWeight: 'bold'}]}> 
                            #{intOutletID}
                            </Text>
                            <Text style={[styles.statusLeave]}> 
                            {strOwnerName} {strContactType != null ? '('+ strContactType + ')' : ''}
                            </Text>
                        </View>
                        <View style={{ flex: 1, flexDirection: 'row', flexBasis: '30%', marginRight: 10, marginTop: 25 }}>
                            <TouchableOpacity onPress={() => Linking.openURL(`mailto:${strEmailAddress}`)}><Image style={[styles.envelopStyle]} source={message} /></TouchableOpacity>
                            <TouchableOpacity onPress={() => Linking.openURL(`tel:${strMobileNumber}`)}><Image style={[styles.telephoneStyle]} source={telephone} /></TouchableOpacity>
                        </View>
